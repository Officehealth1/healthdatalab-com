...
Most recent entry: 2026-07-17 — V2 v0.47.76 (**Stage-2 "Missing distilled_why" root-caused + fixed — DEPLOYED to STBY; FULL E2E PASS on the NEW dedicated STBY Make scenario (all 5 flows: stage2 happy + graceful, stage1, draft, final — 6/6 executions Success, 0 errors, cleanup byte-identical); LIVE legs gated**). Rev 2 same day: 30-day age floor on the never-fired retry belt (STBY preview showed 8 legacy fixture rows the belt would have swept — on LIVE that meant day-one burst burns; tests 34/34, commit `e6b4cfd`). STBY is now on its OWN Make scenario "STBY — Longevity v2 — v28 (email-fenced)" (hook `r2tctgsg…`, all 5 wp-config constants repointed, backup `/root/wp-config.php.bak-pre-v28-hook-*`; all 7 email modules hard-fenced to team+stby@irislab.com). ⚠ E2E blocker found+fixed: imported scenario was toggled OFF (webhooks 200-Accepted but never process, no queue replay) — activation is a REQUIRED import step. RCA: NOT an empty webhook (887-char vision_text, hash match, 200 Accepted) and NOT truncation (deterministic replay: 268/4096 tokens, valid JSON) — Claude deliberately returns `distilled_why:""` for input with no first-person WHY (row 237 was a third-person audio practice recording) and Make module 88's required field hard-errors. WP guard: submit fire needs ≥10 trimmed chars (`STAGE2_MIN_VISION_LEN`); not-ready submit claims completion but DEFERS the fire to the transcript's auto-save (atomic fired_at claim); retry cron now also recovers never-fired-but-completed rows. TDD `tests/stage2-fire-guard/` 32 red(12)→green + full regression green. Make side: **v28 blueprint** (`mockups/make/…v28 (stage2 empty-why guard)…json`) — module-67 route filter (vision_text exist + len≥10) + module-70 filter (distilled_why exist + len>0 → empty WHY ends gracefully, WP retries); module 88 stays strict. Scenario separation runbook `hdl-longevity-v2/docs/plans/MAKE-STBY-DEDICATED-SCENARIO-2026-07-17.md` (STBY = own scenario/new hook, all 5 STBY HDLV2_MAKE_* constants move; LIVE = in-place import, hook unchanged). Make round-trip E2E BLOCKED on Quim's import. Commit `f791625` on `fix/audio-p0-2026-07-15`. Details in `docs/CLAUDE-CHANGELOG.md`.

Previous entry: 2026-07-16 — V1 v1.0.2 (**per-request schema tax removed + client-delete-revokes-V2-invite fix — DEPLOYED to STBY + verified (TDD 11+11, regressions 52/9/20/3, E2E revoke→410, byte-identical cleanup); LIVE untouched, needs "push to live"**). From the same-day invite RCA: `init()` ran 4× dbDelta + 4× SHOW TABLES + ~10 log lines EVERY request (20,964 lines/day) — now gated on `health_tracker_db_version` option vs `DB_VERSION` const (bump it on any V1 schema change); chatty logs WP_DEBUG-gated. V1 delete now revokes the client's `pending/opened` V2 widget invites (SHA2 email-hash match, table-existence guard) so a deleted client's emailed auto-login link dies. Same session: STBY disk 89%→56% (journal vacuum, pre-July rotated logs, Feb-21 provisioning artifacts; rollback backups kept). LSAPI_CHILDREN deliberately unchanged. V1 branch `perf/v1-schema-gate-invite-revoke-2026-07-16` (`e7f0ef8`+`6f81cb9`). Details in `docs/CLAUDE-CHANGELOG.md`.

Previous entry: 2026-07-16 — V2 v0.47.75 (**launch flag for scheduled client campaigns — the 0.47.74 stack now deploys SILENT; DEPLOYED to STBY + live-verified 12/12, LIVE untouched and lands OFF by construction**). Option `hdlv2_ff_client_campaigns` (absent = OFF, joins the existing `hdlv2_ff_*` family) gates the outbound email leg of the ONLY 3 scheduled crons that email a real client — `hdlv2_checkin_reminder`, `hdlv2_quarterly_review` (client leg only; Matthew's prac nudge stays), `hdlv2_weekly_flight_plan` (plan still generates, only the "ready" email is flagged). `HDLV2_Env::client_campaign_gate()` composes the 0.47.74 env gate AND the flag. Practitioner nudges + transactional client mail (finalise → report; check-in → plan) deliberately out of scope. **Launch = `wp option update hdlv2_ff_client_campaigns 1`.** TDD 25 (suite 101) + regression green. Details in `docs/CLAUDE-CHANGELOG.md`.

Previous entry: 2026-07-15 — V2 v0.47.74 (**STBY email safety — whitelist mail guard mu-plugin (belt) + `HDLV2_Env::is_live()` gates on all 8 recurring crons' outbound legs (braces); DEPLOYED to STBY + live-verified 8/8; inert on LIVE by construction (`is_live()` true), nothing deployed to LIVE**). Trigger: 2026-07-14 STBY emailed 2 REAL clients via the revived check-in cron (mail-redirect toggle was pass-through; WP-Cron fires every minute via `/etc/cron.d/hdlv2-wp-cron` despite `DISABLE_WP_CRON`). Guard fails CLOSED on a re-clone (env AND hostname must both say live); manual-test override `hdlv2_allow_staging_side_effects`; no cron code deleted. TDD `tests/email-safety/` 76 assertions + full regression green. 0.47.73 skipped (claimed by uncommitted audio WIP). Details in `docs/CLAUDE-CHANGELOG.md`.

Previous entry: 2026-07-14 — V2 v0.47.69 (hotfix branch) / v0.47.70 (bundle) (**idle-dashboard self-429 fix — DEPLOYED to STBY (bundle variant 0.47.70) + STBY-verified; LIVE leg = hotfix 0.47.69 pending "push to live"**). RCA 2026-07-13 (read-only, LIVE): TIER_READ 200/hr is keyed per WP user and shared by every open tab+device; the client-list 60s fallback poll pair (`/dashboard/clients` + `/widget/leads/pending`) consumed 120 reads/hr PER TAB, so ≥2 idle tabs drained the practitioner's own budget — LIVE logged 114 blocks (all tier=read, exactly those two routes, matthew=122 across 2 IPs/3 tabs), flaring at each hourly window tail; the v0.20.0 global fetch wrapper then modal'd "Give us a moment" on an idle screen. The 4s digest poll was NOT the consumer (unmapped → null tier → unmetered for logged-in). Fix: **[1]** fallback tick full-fetches ONLY when the digest advanced or data >5min stale (idle 120→~20 reads/hr/tab); null roster on 429 keeps the rendered list; **[2]** X-HDLV2-Bg-marked background polls NEVER open the modal (pill only; user-action 429s still modal); **[3]** shared Retry-After cooldown (`hdlv2RateLimit.isCoolingDown()`) armed on any V2 429 — enhancer + practitioner-dashboard 20s + nav-bar 4s pollers all stop until it elapses; **[4]** authenticated-practitioner GETs on the two roster routes consume a new `prac-read` bucket 1200/hr/user (policy `prac_read_config()`, middleware branch BEFORE the read consume; anonymous/token limits byte-untouched, token precedence stands); **[5]** `loadStatus()` now sends a localized REST nonce (`hdlv2RateLimitCfg`) so the status probe counts to the real user; `log_block()` logs `user:<id>` / truncated token (full 64-hex credential no longer written to logs). TDD red→green: `tests/rl-prac-read/` 15 + `tests/rl-idle-poll/` 12+7 (real sources evaluated); regressions green on both variants (token-expiry 26, digest, stage2-retry 16, progress-point, tenancy, iris 87; bundle also action-unify 22+41+6 + resend-link 38+15+22 + inbox). Also fixed stale `scenario-roster-resend.php` fixtures (3/7→7/7, flagged in drift-recovery notes — fixtures now seed the resolve_resend_status tables via a 'funnel' knob). STBY live proof: `/dashboard/clients` as prac returns `X-RateLimit-Tier: prac-read` limit 1200 (bucket shared with pending-leads); other reads stay `read` 200; curl burst as prac 206 → 429 at request 201 with `Retry-After` + STBY OLS log `[HDL-RL BLOCK tier=read id=user:206 …]` (new identity format live); 3-tab idle window (prac 122). ⚠ FOUND during verify: **STBY had `hdlv2_rate_limit_disabled_tiers='all'`** — every RL tier bypassed on STBY (why this class of bug never reproduces there); enforcement enabled for the test window, standing value needs a decision (LIVE runs fully enforced). Branches: hotfix `fix/rl-idle-dashboard-2026-07-14` `b6f2c28` (off d1d6624 = LIVE 0.47.59 tree, for the standalone LIVE deploy AHEAD of the gated dashboard bundle); bundle carry `47b5573` + test fix `13f0f13` on `fix/button-qa-gaps-2026-07-13` (version resolved to 0.47.70; also fixes the 0.47.66-header/0.47.68-constant drift). §B2 (digest 4s→12s + micro-cache) remains a SEPARATE server-load task — the digest is unmetered, so it was never this bug.
