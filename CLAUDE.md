...
Most recent entry: 2026-07-11 — V2 v0.47.58 (**Stage-2 retry cron dead-row fix, DEPLOYED to STBY + verified; LIVE pending "push to live"**; branch `fix/v2-digest-and-progress-point`, commit `2977bb2`). `run_stage2_extraction_retry()`'s candidate SELECT re-fired soft-deleted / token-expired stuck rows to Make ~2×/week forever — the callback then always 404'd "Assessment not found" (`get_progress_by_token()` enforces `deleted_at IS NULL AND token_expires_at > UTC_TIMESTAMP()` since B4; the module-86 Bearer was never the issue, it matches both servers' `HDLV2_MAKE_CALLBACK_SECRET`). Fix adds the same two predicates to the SELECT (NULL expiry fail-closed). Documented tradeoff: expiry also gates the attempt-3 local fallback; recovery = practitioner re-issue (refreshes expiry on the same row, re-arms cron). Tests `tests/stage2-retry/` red→green ×14; STBY verified with 4 disposable rows (old query 4, new query 1-valid; real cron run clean; baseline restored); both servers had 0 stuck rows at deploy. 4-lens review PASS WITH WARNINGS (the documented tradeoff). Full suite green.
